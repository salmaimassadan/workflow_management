...
@section('breadcrumb', 'All Responses')

@section('content')
<head>
    <title>Responses Management</title>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="d-flex mb-3">
                        ...
                        <form id="searchForm" action="{{ route('answers.index') }}" method="GET" class="flex-grow-1">
                        ...
                        <a href="{{ route('answers.download') }}" class="btn btn-outline-info" id="downloadPdfButton">
                    ...
                    <div class="table-responsive">
                        <table class="table">
                            ...
                            @foreach($answers as $answer)
                                <tr>
                                    <td>{{ $answer->courrier->reference }}</td>
                                    <td>{{ $answer->creator->name }}</td>
                                    <td>{{ $answer->content }}</td>
                                    <td>{{ $answer->created_at }}</td>
                                    <td>
                                        <a href="{{ route('answers.show', $answer->id) }}" title="View Response"><button class="btn btn-link btn-sm"><i class="bi bi-eye-fill"></i> View</button></a>
                                        <form method="POST" action="{{ route('answers.destroy', $answer->id) }}" accept-charset="UTF-8" style="display:inline">
                                            {{ method_field('DELETE') }}
                                            {{ csrf_field() }}
                                            <button type="submit" class="btn btn-outline-danger btn-sm" title="Delete Response" onclick="return confirm(&quot;Confirm delete?&quot;)"><i class="bi bi-calendar2-x"></i> Delete</button>
                                        </form>
                                    </td>
                                </tr>
                            @endforeach
